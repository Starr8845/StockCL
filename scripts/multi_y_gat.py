...
from base_models import MemoryModule, ALSTMModel, GRUModel, TCNModel, Transformer, GATModel
...
class GAT(Model):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if base_model=="GATModel":
            self.base_model = GATModel(d_feat=6,hidden_size=64,num_layers=2,dropout=0.7,base_model="LSTM").to(self.device)  # dropout需要注意 benchmark里面为0.7
        else:
            raise NotImplementedError("no {} base model!".format(base_model))
        self.logger.info("model:\n{:}".format(self.base_model))
        params = [{"params":self.base_model.parameters()}]
        ...
        if self.use_memory_net:
            ...
            self.memory_net = MemoryModule(hidden_size=mem_hidden).to(self.device)  
    ...
    def train_epoch(self, epoch, x_train, y_train, y_con, y_con_config={"y_con_type":"multi_y_contrast_loss","use_multi_y":False, "loss_weight":"all1", "metric":"IC"}):
        ...
        for idx, count in zip(daily_index, daily_count):
            ...
            batch_ranking = ranking[batch]
            repres = self.base_model.get_repre(feature)
            loss=0
            if self.use_memory_net:
            ...
            pred = self.base_model.predict(repres)
            loss += self.loss_fn(pred, label)
            tra_reg_loss+=loss.item()
            
            if y_con:
                multi_y = multi_y_values[batch]
                if y_con_config["y_con_type"]=="multi_y_contrast_loss":
                    ...
                    final_x, final_y = sample_positive(batch_x=x_train_values[batch], 
                                                       batch_y=y_train_values[batch], 
                    ...
                    repres_final = self.base_model.get_repre(final_x)
                    if self.use_memory_net:
                    ...
                    positive_num+=positive_num_
                    negative_num+=negative_num_
                loss += y_con_loss/5     # 使用IC 时 除以1, 使用weightedl1时，除以10
                additional_loss+=y_con_loss.item()/5
            
            self.train_optimizer.zero_grad()
            loss.backward()
            torch.nn.utils.clip_grad_value_(self.base_model.parameters(), 3.0)
            self.train_optimizer.step()

        positive_num/=len(x_train_values)
        negative_num/=len(x_train_values)
        sample_num = len(x_train_values)
        tra_reg_loss, additional_loss, tra_loss_constraint = tra_reg_loss/(sample_num//self.batch_size+1), additional_loss/(sample_num//self.batch_size+1),tra_loss_constraint/(sample_num//self.batch_size+1)
    ...
    def test_epoch(self, data_x, data_y, data_split):  
        ...
        for idx, count in zip(daily_index, daily_count):
            ...
            IC, RIC = self.cal_my_IC(pred.detach().cpu().numpy().squeeze(), label.detach().cpu().numpy().squeeze())
            my_ICs.append(IC)
            my_RICs.append(RIC)
    ...
    def ana_eval(self, sr, recorder, x_test, benchmark):
        ...
        for begin in range(sample_num)[:: self.batch_size]:
            ...
            with torch.no_grad():
                pred = self.base_model(x_batch).detach().cpu().numpy()
    ...
    def fit(
        ...
        for step in range(self.n_epochs):
            ...
            if valid_ICs['IC'] > best_score:
                ...
                stop_steps = 0
                best_epoch = step
                best_param = copy.deepcopy(self.base_model.state_dict())
            else:
                stop_steps += 1
        ...
        self.base_model.load_state_dict(best_param)
